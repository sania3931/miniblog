...
@section('content')
    <div class="section__content section__content--p30">
        <div class="container-fluid">
            <form action="{{ url(Auth::user()->role . '/artikels/' . $artikel->id) }}" method="post"
                    ...
                    <div class="col-8">
                        <div class="card">
                            ...
                            <div class="card-body card-block">
                                ...
                                <div class="tab-content" id="myTabContent" style="margin-top: 2rem;">
                                    <div class="tab-pane fade active show" id="indonesia" role="tabpanel"
                                        ...
                                        <div class="col-12">
                                            <textarea name="content" class="tinymce" id="" cols="90" rows="10">{!! $artikel->content !!}</textarea>
                                        </div>
                                    </div>
                                    <div class="tab-pane fade" id="english" role="tabpanel" aria-labelledby="english-tab">
                                        ...
                                        <div class="col-12">
                                            <textarea name="content_en" class="tinymce" id="" cols="90" rows="10">{!! $artikel->content_en !!}</textarea>
                                        </div>
                                    </div>
                                </div>
                            ...
                            {{-- <div class="card-body card-block">
                                <div class="row form-group">
                                    <div class="col-12">
                                        <label class=" form-control-label">@lang('form.title')</label>
                                    </div>
                                    <div class="col-12">
                                        <input type="text" id="text-input" name="title" placeholder="@lang('form.enterTitleHere')"
                                            class="form-control @error('title') is-invalid @enderror"
                                            value="{{ $artikel->title }}">
                                        @error('title')
                                            <div class="invalid-feedback">
                                ...
                                <div class="col-12 mb-3">
                                    <button type="button" class="btn btn-success mb-1 btn-add" data-toggle="modal"
                                        data-target="#largeModal">@lang('button.addImage')</button>
                                </div>
                                <div class="col-12">
                                    <textarea name="content" class="tinymce" id="" cols="90" rows="10">{!! $artikel->content !!}</textarea>
                                </div>
                                <div class="card-footer">
                                    <button  name="status" type="submit" class="btn btn-success btn-sm" value="1">@lang('button.save')
                                    </button>
                                    <button  name="status" type="submit" class="btn btn-outline-success btn-sm" value="0">@lang('button.saveToDraft')
                                    </button>
                                    <a href="{{ url(Auth::user()->role.'/artikels') }}" class="btn btn-danger btn-sm">
                                        <i class="fa fa-chevron-left"></i> @lang('button.back')
                                    </a>
                    ...
                    <div class="col-4">
                        <div class="card">
                            ...
                            <div class="card-body card-block">
                                <div class="row row-form">
                                    <div class="col-12">
                                        ...
                                        @enderror
                                        <img src="{{ asset('images/' . $artikel->featured_image) }}" alt=""
                                            class="img-fluid">
                                    </div>
                                </div>
                            </div>
                            <div class="col-12 m-b-30">
                                ...
                                <select name="category_id" id="select" class="form-control">
                                    @foreach ($category as $row)
                                        <option
                                            value="{{ $row->id }}"@if ($row->id == $artikel->category_id) selected @endif>
                                            {{ $row->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-12 m-b-30">
                                ...
                                <ul class="list-unstyled d-inline-block mt-3" id="recant-tags">
                                </ul>
                                <style>
                                    .tag-recent {
...
@section('script')
    <script>
        ...
        //         $.ajax({
        //             type: "GET",
        //             url: `{{ url('Admin/artikels/${id}/create') }}`,
        //             cache: false,
        //             success: function(response) {
        //                 $('.modal-content').html(response);
        //                 $('#largeModal').modal('show');
        ...
        function filltags() {
            $.ajax({
                type: "GET",
                url: "{{ url(Auth::user()->role . '/recanttags') }}",
                data: {
                    post_id: '{{ $artikel->id }}'
                },
                success: function(response) {
                    $("#recant-tags").html(response);
                }
            });
        }

        function deletetags(e, id) {
            e.preventDefault();
            var url_tag = "{{ url(Auth::user()->role . '/tag') }}/" + id
            $.ajax({
                type: "DELETE",
                url: url_tag,
                headers: {
